...
function resetSliceFixture(): void {
  clearCarrierAmountSlicesForTest();
  Game.time = 10_000;
  Game.creeps = {};
}
...
describe("CarrierAmountSlicePort", () => {
  ...
  it("validates inputs and isolates owned task, step, and claimant budgets", () => {
    resetSliceFixture();
    {
  ...
  it("reconciles execution handles, owner deletion, actor liveness, and runtime identity", () => {
    resetSliceFixture();
    {
    ...
    resetSliceFixture();
    {
      ...
      expect(claim(refreshedRef, refreshedSteps, "first", "carrier-d", 700)?.amount)
        .toBe(700);
    ...
    resetSliceFixture();
    {
      ...
      const inherited = claim(inheritedRef, steps, "step", "constructor", 1_000);
      inherited?.commit();
      ...
      Game.creeps = {};
      const inheritedProto = claim(protoRef, steps, "step", "__proto__", 1_000);
      ...
      const own = claim(liveThenDeadRef, steps, "step", "toString", 1_000);
      own?.commit();
