...
- [x] 1.3 在现有 owner 分支下定义局部 versioned control/rollback、latest intent、TransferContract、CapacityLease、StageWorkClaim、market proposal 与 runtime adapter 类型；cfg control 与首片 raw data Memory 保持 `schemaVersion:1`，data 使用 `compact-v1` canonical tuple wire，但公开 decoded DTO 与领域语义不变；CPU/因果投影升级后的 runtime 使用 `schemaVersion:2`，不得直接扩大冻结的四个 Memory 根声明指纹
...
- [x] 8.2a 通过局部 adapter 扩展 `Memory.runtime.resourceControl.logistics` 的 Shadow v2 投影（cfg/data v1 不变），输出 mode/schema、in-scope/out-of-scope、intent freshness/revision、legacy 配对率、各 comparator 维度 match/difference reason、predicted staging、candidate/index/CPU/Memory，以及 `effectiveAuthority`、active contract/lease/claim store 和可观察 actor/claim/journal/invariant 状态；v2 `cpu` 固定 exact `{attributionVersion,sampleTick,measurementAvailable,producerUsed,consumerUsed}`，runtime v1 仅历史展示且 gate-ineligible；禁止把声明常量或净状态伪装为跨模块瞬时 attempt 计数，也禁止伪造 contract/lease/claim 已恢复
...
- [ ] 8.5a 在固定 live-like fixture 上证明 Shadow 房间事实与 matcher 不进行每房全表扫描；用 module-local tick-bound segmented meter 在既有 logistics runtime owner 内分别计量所有仅由 Shadow 引入的 Synthesis `producerUsed` 与 ResourceControl 子区间 `consumerUsed`，不得新增 analytics owner；consumer 闭包必须覆盖 provisional projection/fixed-point/trim，只允许最终归因标量及其一次不可递归 self-observation seal 位于闭包外；锁定 `shadowUsed=producerUsed+consumerUsed` 只作归因、正式 `gateUsed=完整 ResourceControl phase+producerUsed` 且不重复计 consumer；同 tick post `gateUsed` p95 不高于 P0 基线的 110%（增幅不超过 10%），且 logistics data+runtime 的 UTF-8 JSON 序列化字节数合计在所有样本中不超过 32 KiB
...
- [ ] 9.1a 运行 intent store、Synthesis 写前冻结/legacy decision 配对、纯 matcher/comparator、ResourceControl/P0 headroom、Hub 不变回归、monitor 兼容、disabled-vs-shadow 非 logistics 状态差分、send/deal mock 无新增调用、`effectiveAuthority=legacy`/active store 零值/无 Logistics Shadow actor-claim-journal，以及 CPU/Memory 门槛聚焦测试并修复回归；必须覆盖 cfg/data v1 与 runtime v2 边界、runtime v1 仅历史展示且不得进入 10+100/CPU gate、v2 `cpu` exact 五字段且 `attributionVersion=2/measurementAvailable=true`、module-local producer/consumer 分段归因、无第三 analytics owner、gate 不双计、runtime/data epoch+byte 两轮 bracket coherent retry（硬上限 4 runtime + 2 data）、CPU history 中选定 sample tick 与 runtime attribution 对齐且禁止 `summary.latestTick` 拼接、no-route 不得标 unsafe、legacy route 时 source disposition 必填且 no-route 时 absent/null，以及缺因果 fail closed；本项不以硬编码零值声称排除了未 instrument 的瞬时 attempt
...
- [ ] 9.4 在 1.1a、`production-logistics-liveness` deploy/live gate，以及 8.1 的 disabled-vs-shadow 可观察状态差分与 send/deal mock 无新增调用完成后，允许部署零 authority 的 `synthesis_room`-only Shadow，不以 `terminal-headroom-recovery` 6.4 尚未关闭阻止纯只读比较；剔除至少 10 warmup tick 后收集至少 100 个连续 measured tick，确认每个 runtime/data/CPU 样本 coherent 且同 tick、in-scope legacy 全部配对并具有可由冻结输入验证的 decision delta/方向/主因，legacy route 时带 source disposition、legacy no-route 时 disposition 为 absent/null N/A，out-of-scope 不静默遗漏，donor/route/priority/coverage/headroom/predicted-staging 差异可解释，且持续为 `effectiveAuthority=legacy`、active contract/lease/claim store 为零、无 Logistics Shadow actor/claim/journal 记录和可观察 invariant violation；正式 `gateUsed=完整 ResourceControl phase+producerUsed` 的 post p95 必须 `<= pre * 1.10`，不得重复加已含于 outer phase 的 `consumerUsed`，每 tick logistics data+runtime UTF-8 JSON 字节数必须 `<= 32768`。任何改变归因、差异因果或 coherent-read 语义的新 bundle 都必须重新冻结同口径基线并从零重跑 10+100，旧窗不得拼接或倒填；该 live gate 只证明可观察结果，不得被表述为已经排除未 instrument、发生后又回滚/释放/失败的瞬时 attempt
