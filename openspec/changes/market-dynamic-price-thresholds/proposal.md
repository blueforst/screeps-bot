...
- **P1.5：生产预留显性化（按资源差异化）**。现有机制：策略表每 lane `laneReserve`（现 7 资源一刀切 100,000）作为保护账本 forecastBuffer 分量，在可售量计算前扣除（live 证据：X/E6N59 protected=100,180、H/E3N59 protected=100,000——出售永不触碰这批库存）。本变将其显性化为**按资源定义的战略预留**：用户按生产实际（boost 链催化剂 X 消耗、合成房反应物、战备）为每资源设定预留值；预留语义不变（每 lane 扣除、生产优先级恒在市场之上），数值调整走 P0 迁移操作（一次 re-sign，无重置）；`surplusRatio` 一律在预留扣除后计算（现有保护账本顺序天然满足）。monitor 投影每资源"预留/实际需求/可售"三层数量，让预留足迹可观察。
