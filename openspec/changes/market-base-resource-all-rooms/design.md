...
- market planning 从 outer 首次 CPU 读、runtime session、scope core、market facts、Shadow batch、inner apply，一直到完整候选 root 的私有注册和最终 precommit，使用同一个 hard CPU ceiling 25；超过后零 WAL/claim/deal，并投影 bounded telemetry。prepared WAL 一旦已由 canonical CAS 成功落盘则优先保留，不能被后续诊断回退覆盖。
...
outer 必须在完整候选 root 与必要的 reset-only fallback root 都完成私有 capability 注册后，紧邻 canonical assignment 再读取一次未截断 CPU，并在该读取之后重新验证 `Memory.data.marketSaleAutomation` 与当前 context 仍逐字指向规划前 exact root。最终 assignment 也必须再次执行 expected-source CAS、捕获 setter 异常并验证写后 object identity；setter 静默拒绝或替换为并发 root 时不得二次写回旧 source，更不得让 context 虚报候选 root。CPU 超限、读数无效/回拨、exact CAS 或 setter 失败时，`planComplete=false`；完整 root、Shadow cursor、完整周期、qualification 与 pricing ratchet 一律不得提交。只有本轮纯 Shadow、无 selected、无 prepared WAL，且最多 8 条已采样 lane 的 `incomplete` 事实需要清除既有正向资格时，才可提交独立构造和认证的 reset-only root；它保留规划前 scope cursor、ratchet、ledger、permit、preflight/WAL 安全状态，只把这些 `shadow/qualified+suspended` lane 清零。原本已是零周期 Shadow 的负向 digest 不强迫第二次 canonical replacement；完整 root 的私有注册失败也不得压掉已认证的必要 negative reset，此时 fallback snapshot 必须记录真实 root blocker，不能伪造 CPU cut。

CPU trace 只允许保存在 runtime 投影，并在正常 canonical 提交时镜像到 planning snapshot。它固定记录同一 outer 起点的 `cpuAfterOuterSession/cpuAfterScopeCore/cpuAfterMarketFacts/cpuAfterShadowBatch/cpuAfterInnerApply`，数值上限 100，只保留首个 `cpuCutPhase` 与闭集 `marketFactsDisposition`。inner 入口的第一次 CPU 读数本身就必须初始化未截断 high-water；所有后续 gate 与 telemetry 读取共享该不可回拨 high-water，任一读数下降、无效或已超过 25 后，inner/outer 都不得被较小读数重新打开。第二读开始时必须清空尚未由第二读重新到达的后继里程碑并重置该读 disposition；只有全部前序里程碑均非空时才能写入下一里程碑，partial/incomplete read 的通用结束观测只能更新 raw high-water，不能伪造后继字段。inner 还必须在同一调用栈单独返回未截断 high-water 给 outer 最终门禁；该标量不持久化、不进入 trace/canonical evidence，缺失、无效或小于 trace 最大值时 outer 必须 fail closed。outer 必须在任何 root capability 注册和 fresh CPU callback 之前，把 trace 重建为精确字段快照并复制 high-water primitive；之后不得再从可变 inner 返回对象读取 CPU 证据。由此 trace 数值单调且只允许 trailing null；monitor 对非单调、null-hole、额外字段或越界值整条 fail closed。trace 不进入 canonical hash、permit、scope evidence、pricing、protection、quota、排序或授权。即使最终 root 因 CPU/CAS 被丢弃，monitor 也必须从 runtime 展示本 tick trace。
