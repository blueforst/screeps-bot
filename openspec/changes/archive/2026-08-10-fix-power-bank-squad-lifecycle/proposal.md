...
- 当已到达目标房的就绪替补进入主力剩余寿命安全交接窗口时，原子晋升替补并只退役旧代，避免替补长期占路等待主力失效。
- PowerBank 双人组在同房重新会合或接近 Bank 时动态避让已占用格，并在有意停留时清除旧移动状态，避免缓存路径与交通让路形成活锁。
- 为替补保存独立阶段/位置进展与 blocker；travelling 连续停滞时执行有冷却的移动状态软重置，不再被主力持续攻击产生的任务级进展掩盖。
...
- 同房避让只改变 PowerBank attacker/healer 的移动调用参数与空闲状态清理，不扩大到通用寻路策略。
