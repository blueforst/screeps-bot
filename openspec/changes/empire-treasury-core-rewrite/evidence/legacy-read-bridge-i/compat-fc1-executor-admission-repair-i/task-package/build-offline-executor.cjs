...
const ROOT=path.resolve(__dirname),DELIVERY=path.join(ROOT,'fc1-delivery'),P=require(path.join(DELIVERY,'PACKAGE.json'));
...
function parse(args){const o={};for(let i=0;i<args.length;i+=2){if(!['--repo','--source-ref','--out'].includes(args[i])||!args[i+1])throw new Error('INVALID_ARGUMENTS');o[args[i].slice(2)]=args[i+1];}if(!o.repo||!o['source-ref']||!o.out)throw new Error('REQUIRED_ARGUMENT_MISSING');return o;}
function build(o){
 C.verify();
 ...
 const manifest={kind:'full-cost-FC1-source/v1',expectedSourceTree:tree,files};
 ...
 const inherited=path.join(ROOT,'baseline-executor');
 const result=A.resolve(inherited,out,source,manifest,receipt);
 const policy=JSON.parse(fs.readFileSync(path.join(out,'policy.json'),'utf8'));
 if(policy.newRoundAuthorization?.status!=='WAITING_FOR_EXPLICIT_AUTHORIZATION'||policy.newRoundAuthorization.authorizationId!==null||
