...
if(!assembled){
 ...
 const baseline=fs.readFileSync(path.join(__dirname,'../../baseline-executor/runtime/time-budget.cjs'),'utf8');
 ...
 test('both live entry points receive a fail-closed fresh-authorization guard',()=>{
  ...
  const observe=fs.readFileSync(path.join(__dirname,'../../baseline-executor/tools/observe.cjs'),'utf8');
