...
function fixture(){
 ...
 const files={};for(const name of ['before.txt','new.txt']){const bytes=fs.readFileSync(path.join(repo,name));files[name]={after:{bytes:bytes.length,sha256:sha(bytes)}};}
 fs.writeFileSync(path.join(pkg,'source-manifest.json'),JSON.stringify({expectedSourceTree:p.compatBaseTree,files},null,2)+'\n');
