...
系统 SHALL 将每个同时拥有 Storage 与 Terminal 的己方房间独立于能量状态分类为 `normal`、`pressure` 或 `emergency`。系统 SHALL 在达到配置的低剩余容量阈值时进入压力状态，并且只有 Storage 与 Terminal 都达到配置的恢复水位后才返回 normal。
...
系统 SHALL 仅向同时拥有 Storage 与 Terminal、状态为 normal 且达到配置接收准入水位的己方房间创建新缓解路径。每次发送 SHALL 保留配置的 Storage 与 Terminal 安全缓冲；pressure 或 emergency 房间 SHALL NOT 接收容量缓解库存。

#### Scenario: 接收房通过准入
- **WHEN** 使用默认值时，normal 房间至少有 300,000 Storage 剩余容量和 50,000 Terminal 剩余容量
- **THEN** 该房间可作为新容量缓解路径的接收房

#### Scenario: 发送量受接收端缓冲限制
- **WHEN** 使用默认值时，pending 缓解任务会使接收房 Storage 剩余容量低于 100,000 或 Terminal 剩余容量低于 40,000
- **THEN** 发送量限制为两个安全容量中的较小值，且不得越过任一缓冲

#### Scenario: 没有安全接收房
- **WHEN** 所有其他己方房间均处于压力状态或低于接收准入水位
...
系统 SHALL 从 Storage 与 Terminal 的 Energy 总量计算 donor 可用量，同时保留 donor Energy floor、Terminal Energy reserve、有效预留、出站承诺和交易费用。生存 Energy 支援 SHALL 先于自动容量缓解运行。

#### Scenario: Terminal-heavy donor 可以支援
- **WHEN** donor 的 Energy 总量高于全部受保护 Energy，即使 Storage Energy 单独低于出口阈值
- **THEN** 超出部分可在不越过 donor 保护的前提下发送给生存房间

#### Scenario: Donor 保护优先
- **WHEN** 拟发送 Energy 或对应费用会穿透 donor Energy floor 或 Terminal reserve
- **THEN** 系统减少发送量或跳过发送
