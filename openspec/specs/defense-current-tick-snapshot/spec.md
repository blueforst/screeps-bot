# defense-current-tick-snapshot Specification

## Purpose
规定跨模块 Defense Mode 安全闩的 current-tick 新鲜度、Room Planner revision 代际、危险 hostile 兼容判定与原子失败语义，确保早期和晚期消费者不会把旧状态或半份 snapshot 当作当前安全结论。
## Requirements
