# remote-invader-core-clearance 规范

## Purpose

定义有效外矿 level 0 Invader Core 的单实例自动清理、视野与安全门禁，以及完成后的幂等退役合同。
...
Core 清理模式下的 `remoteDefender` SHALL 仅攻击合法 hostile creep 与 `STRUCTURE_INVADER_CORE`，不得把其他玩家或遗留结构作为结构目标，也不得使用可能波及这些结构的群体攻击。
...
#### Scenario: Core 护卫与玩家结构同时存在
- **WHEN** Core 清理模式下存在多个合法 Invader creep，且攻击范围内同时有玩家结构
- **THEN** 单位对所选 creep 使用单体远程攻击，不得使用 `rangedMassAttack`
